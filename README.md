...
│   ├── net/
│   │   ├── AddressWorker.kt      # GPS坐标转地址
│   │   ├── ImageWorker.kt        # 图片加载与缓存
│   │   └── VehicleDataWorker.kt  # 车辆数据API调用
│   ├── receiver/
│   │   └── ScreenOnReceiver.kt   # 屏幕亮起广播接收
│   ├── ui/theme/                 # Compose UI主题配置
│   ├── util/
│   │   └── EnergyCalculator.kt   # 能耗计算工具
│   ├── widget/
│   │   └── data/VehicleData.kt   # 车辆状态数据模型
│   └── worker/
...
1. **下载安装应用**
   - 下载最新版本的 [MG Linker](https://gitee.com/yangyachao-X/mg-linker/releases)
   - 在手机上允许安装未知来源应用后完成安装
...
详细步骤图示见 HttpCanary 目录下的图片教程 (1.jpg - 10.jpg)
...
| 配置项 | 说明 |
|--------|------|
| **品牌选择** | 选择名爵(MG)或荣威(RW)品牌 |
| **车型选择** | 选择具体车型如MG7、MG5、D7等 |
...
A: 确保使用正确版本的官方App；荣威车型需要抓取特定接口；尝试完整操作一遍App登录流程，操作App上所有的功能点，例如商城，服务，权益，车辆信息等，再重新抓取Token
