...
| 功能模块 | 描述 |
|---------|------|
| **车辆状态监控** | 续航里程、剩余油量/电量、车门锁状态、车内温度、胎压等 |
| **远程车控** | 锁车/解锁、开启后备箱、寻车、空调控制、座椅加热、车窗控制 |
| **桌面小组件** | 4 种尺寸风格，支持深色主题、自定义样式 |
| **蓝牙数字钥匙** | 靠近自动解锁、离开自动落锁、蓝牙直连车控 |
| **灵动岛** | 状态栏悬浮显示车辆状态，支持快捷控制 (模仿 iOS Dynamic Island) |
| **适用车型** |  MG7、MG5、MG4、荣威D7、荣威D5X 等上汽名爵/荣威系列车型 | 

## 安装步骤

1. **下载安装应用**
   - 下载最新版本的[MG Linker](https://gitee.com/yangyachao-X/mg-linker/releases/download/8.0/MG%20Linker.apk)
   - 在手机上允许安装未知来源应用后完成安装
...
4. **添加桌面小组件**
   - 给予应用'桌面快捷方式'权限自动添加
   - 如果不能自动添加，长按手机桌面 → 添加小部件 → 找到MG Linker
   - 选择合适的组件尺寸添加到桌面
   - 配置成功后小组件将自动开始显示车辆数据
## 版本历史
...
| **User Name输入** | 登录自动获取                  |
| **PIN输入**       | 官方APP远程控车密码             |
| **实时同步车机数据**    | 激活车机TBOX上传最新数据          |
| **锁车自动关窗**      | 远控锁车操作同时关窗(需车辆支持远程车窗操作) |
| **隐藏后台**        | 从手机最近任务中隐藏              |
| **靠近解锁/离开落锁**   | 后台检测蓝牙信号自动解锁/上锁         |
| **检查更新**        | 手动触发版本更新检查              |
...
| **依赖注入** | 手动依赖注入 (无 Hilt/Dagger) |
| **设计规范** |遵循 HyperOS / MIUI + 视觉风格大圆角卡片 (28dp)、胶囊按钮、极简配色 + 支持动态主题切换，实时生效 |
### 项目结构
...
### Q: 为什么没有远程/蓝牙控车代码
A: 控车代码是逆向的官方app，包含了官方源代码与包，无法开源，如果各位有需要，需自行逆向

## 💖鸣谢

本项目的发展离不开大家的支持使用，优化建议以及反馈。感谢每一位朋友的慷慨捐赠，你们的支持是我持续维护和迭代的最大动力！

（媳妇儿说我不干正事，整天搞这个app，不陪她打游戏哈哈哈，我放她的收款码（胡瑶）)
