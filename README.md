...
│   ├── net/
│   │   ├── VehicleDataWorker.kt  # [网络] 核心网络层 / 负责调用上汽接口获取车辆数据 / Token 管理
│   │   ├── AddressWorker.kt      # [网络] 逆地理编码服务 / 将 GPS 坐标转换为中文物理地址
│   │   └── ImageWorker.kt        # [网络] 图片加载服务 / 处理车辆渲染图的下载与缓存
│   ├── ui/
│   │   ├── LogViewerScreen.kt    # [界面] 日志中心页面 / 支持实时刷新、过滤日志、开始/停止录制
│   │   ├── MainComponents.kt     # [界面] 配置页组件 (MGConfigScreen) / 输入框 / 更新弹窗 / 下拉选择器
│   │   └── theme/                # [界面] UI 主题配置 (遵循 Material3 & HyperOS 风格)
│   ├── util/
│   │   └── EnergyCalculator.kt   # [算法] 能耗计算工具 / 估算剩余电量与续航关系
│   └── worker/
│       └── WidgetUpdateWorker.kt # [后台] 定时任务 (WorkManager) / 负责在后台周期性拉取数据并刷新 Widget
...
| 版本  | 更新内容                        |
|-----|-----------------------------|
| 3.1 | 最新稳定版本-1.增加日志中心-左滑；2.优化一些逻辑 |
