...
├── app/src/main/
│   ├── java/com/my/mg/
│   │   ├── MainActivity.kt           # [核心入口] App 主容器 / Compose 宿主 / 权限请求处理
│   │   ├── MainViewModel.kt          # [架构-VM] MVVM ViewModel / 管理 UI 状态流 / 协调后台任务与数据存储
│   │   ├── MGWidget.kt               # [小组件] 4x2 标准小组件 Provider / 负责接收广播与更新 UI
│   │   ├── MGWidgetSmall.kt          # [小组件] 2x2 小型小组件 Provider
│   │   ├── data/
│   │   │   ├── VehicleData.kt        # [模型] 车辆核心数据实体 (续航、电量、门窗、胎压、定位等)
│   │   │   ├── MainUIState.kt        # [模型] 主页 UI 状态 (Loading、车辆信息、错误信息)
│   │   │   ├── WidgetContextData.kt  # [模型] 小组件渲染上下文 / 聚合车辆数据与更新时间
│   │   │   ├── FuelRecord.kt         # [模型] 能耗/加油记录实体 / 用于计算平均能耗
│   │   │   ├── CarConfig.kt          # [配置] 静态车辆配置 (车型 ID、颜色代码映射表)
│   │   │   └── GiteeRelease.kt       # [模型] 版本更新接口响应结构
│   │   ├── log/
│   │   │   └── LogcatHelper.kt       # [工具] 日志系统 / 负责抓取 Logcat、写入文件及 UI 展示流
│   │   ├── net/
│   │   │   ├── VehicleDataWorker.kt  # [网络] 核心车辆 API 服务 / 登录鉴权 / 获取车辆状态 JSON
│   │   │   ├── AddressWorker.kt      # [网络] 逆地理编码服务 / 将 GPS 坐标转换为中文物理地址
│   │   │   └── ImageWorker.kt        # [网络] 车辆渲染图服务 / 下载并缓存不同角度的车辆图片
│   │   ├── ui/
│   │   │   ├── MainComponents.kt     # [界面] 主页核心组件 / 车辆控制卡片 / 登录配置表单
│   │   │   ├── LogViewerScreen.kt    # [界面] 日志监控页 / 实时日志流展示与控制
│   │   │   └── theme/                # [界面] Compose 主题配置 (Color, Type, Theme)
│   │   ├── util/
│   │   │   ├── EnergyCalculator.kt   # [算法] 能耗计算引擎 / 混动油电折算逻辑
│   │   │   └── FuelRecordStore.kt    # [存储] 本地持久化 / 管理能耗历史记录 (SharedPreferences/File)
│   │   └── worker/
│   │       ├── WidgetUpdateWorker.kt # [后台] 核心定时任务 / 周期性拉取数据并刷新所有 Widget
│   │       └── EnergyRecordWorker.kt # [后台] 数据采集任务 / 定期记录车辆状态以供能耗分析
│   └── res/
│       ├── layout/                   # [布局] RemoteViews XML 布局 (Compose 无法直接用于 Widget 布局)
│       │   ├── mg_widget.xml         # [布局] 4x2 主布局
│       │   ├── mg_widget_small.xml   # [布局] 2x2 小布局
│       │   ├── mg_widget_icon.xml    # [布局] 1x1 图标布局
│       │   └── ...                   # 其他样式变体 (style1, style2)
│       ├── xml/                      # [配置] Widget 元数据 (尺寸、预览图、更新频率定义)
│       ├── drawable*/                # [资源] 车辆状态图标 (车门、车窗、轮胎、电量图标等)
│       └── values/                   # [资源] 颜色、字符串、主题定义
...
1. **下载安装应用**
   - 下载最新版本的[MG Linke](https://gitee.com/yangyachao-X/mg-linker/releases/download/3.2/MG%20Linker.apk)
   - 在手机上允许安装未知来源应用后完成安装
...
应用内置能耗计算功能，根据车辆配置自动计算油耗/电耗：

```kotlin
EnergyCalculator.calculate(
    fuelRange: Double,           // 燃油续航(km)
    fuelCapacity: Double,        // 油箱容量(L)
    fuelLevel: Double,           // 燃油百分比
    batteryRange: Double,        // 电池续航(km)
    batteryCapacity: Double,     // 电池容量(kWh)
    batteryPackPrc: Double       // 电池百分比
): ConsumptionResult
```
...
| 字段 | 说明 |
|------|------|
| displayText | UI显示文本 (例如 " / 5.6L/100km") |
| rawValue | 原始数值 |
